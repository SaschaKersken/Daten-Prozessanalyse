...
class DistanceChromosome(Chromosome):
    ...
    def random_instance(cls):
        ...
        shuffle(cities_copy)
        return DistanceChromosome(cities_copy)

    # Mutation: zwei zufällige Städte tauschen
    def mutate(self):
        rand_index_1 = randint(0, len(self.cities) - 1)
        rand_index_2 = randint(0, len(self.cities) - 1)
...
if __name__ == '__main__':
    ...
    for i in range(0, 100):
        population.append(DistanceChromosome.random_instance())
    # Genetischen Algorithmus initialisieren und laufen lassen
    genetic = Genetic(population, 0.0002525252525, 100, 0.7, 0.5)
    best = genetic.run()
    # Die Lösung ausgeben
